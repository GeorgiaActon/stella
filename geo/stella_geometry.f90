module stella_geometry
  ...
  public :: get_dzed, get_dzed_complex
...
contains
  ...
  subroutine init_geometry (change_p)
    ...
    if (proc0) then
       ...
       case (geo_option_local)
          ...
          if (present(change_p)) then
             call read_local_parameters (geo_surf, change_p)
          else
...
!    twist_and_shift_geo_fac = geo_surf%shat*(gds21(1,-nzgrid)/gds22(1,-nzgrid)-gds21(1,nzgrid)/gds22(1,nzgrid))
  ...
  subroutine get_dzed (nz, dz, f, df)
    ...
    real, dimension (-nz:nz),intent (out) :: df
  ...
  subroutine get_dzed_complex (nz, dz, f, df)

    implicit none

    integer, intent (in) :: nz
    real, dimension (-nz:), intent (in) :: dz
    complex, dimension (-nz:), intent (in) :: f
    complex, dimension(-nz:nz), intent (out) :: df
  ...
  end subroutine get_dzed_complex
